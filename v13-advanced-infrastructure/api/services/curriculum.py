"""Curriculum generation service for StudyBuddy v13.

Generates learning curricula for any topic using AI.
Users can also upload their own topic lists in markdown format.

v13 adds multi-model support via ModelRouter for flexible model selection.
"""
...
async def generate_curriculum(
    ...
    if model_router:
        ...
        logger.info(f"Generated curriculum using {model_used} (fallback={was_fallback})")
...
async def generate_curriculum_from_documents(
    ...
    """
    # Build the prompt
    formatted_prompt = CURRICULUM_FROM_DOCUMENTS_PROMPT.format(
    ...
    if model_router:
        ...
        logger.info(f"Generated curriculum from docs using {model_used} (fallback={was_fallback})")
        return response_text
