...
- Adding LangGraph native memory with langmem
- **Persistence strategy**: JSON files or SQLite for development (simple, visible, no extra infrastructure)
  - PostgreSQL will be used in v12 for production multi-user deployment
  - Using LangGraph's BaseStore with file/SQLite backend
- Creating memory tools (create_manage_memory_tool, create_search_memory_tool)
- Adding the **Scheduler Agent** for spaced repetition review sessions
- Implementing the SM-2 spaced repetition algorithm:
  - Tracking card review history (requires memory persistence)
  - Calculating optimal review intervals
  - Adjusting ease factors based on performance
- Tracking learning history across sessions:
  - What concepts you've studied
  - What you struggle with vs. master quickly
  - Your preferred learning style (detailed vs. concise)
  - Topics you want to focus on
  - Flashcard review performance over time
- Memory connecting both modes: tutoring informs practice, practice informs tutoring
- Implementing "Remember that I'm studying for AI Engineering certification" persistence
- Testing memory across sessions (restart server, verify data persists)
- Managing user-specific memory namespaces
