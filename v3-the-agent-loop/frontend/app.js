...
async function checkIndexingStatus() {
    try {
        ...
        if (status.indexing_complete) {
            versionTag.textContent = `StudyBuddy v3 · ${status.documents_indexed} stories indexed (${status.chunks_in_db} chunks)`;
            indexingComplete = true;
        } else {
            const currentFile = status.current_file ? ` "${status.current_file}"` : '';
            versionTag.textContent = `StudyBuddy v3 · Indexing${currentFile}... (${status.chunks_in_db} chunks)`;
            // Poll again in 500ms for smoother updates
            setTimeout(checkIndexingStatus, 500);
        }
    } catch (error) {
        // Server not ready yet, try again
        versionTag.textContent = `StudyBuddy v3 · Connecting...`;
    ...
    try {
        ...
        if (data.reasoning) {
            ...
            reasoningDiv.innerHTML = `
                <button class="reasoning-toggle">Show reasoning</button>
                <div class="reasoning-content hidden">
                    <pre>${escapeHtml(data.reasoning)}</pre>
                </div>
            `;
            loadingDiv.parentElement.appendChild(reasoningDiv);

            // Toggle click handler
            const toggle = reasoningDiv.querySelector('.reasoning-toggle');
