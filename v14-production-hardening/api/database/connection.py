"""Database connection and session management for v14."""
...
engine = create_engine(
    DATABASE_URL,
    pool_pre_ping=True,
    pool_size=20,           # v14: increased from 10
    max_overflow=30,        # v14: increased from 20
    pool_timeout=30,        # v14: explicit timeout
    pool_recycle=1800,      # v14: recycle connections every 30 minutes
)
...
def _run_v12_migrations(engine):
    """Add v12 authentication columns to the users table.

    SQLAlchemy's create_all() creates new tables but does NOT alter existing ones.
    This runs ALTER TABLE for any missing v12 columns on the users table.
    """
    inspector = inspect(engine)
    ...
    if migrations:
        with engine.begin() as conn:
            for sql in migrations:
                conn.execute(text(sql))
                print(f"  Migration: {sql.split('ADD COLUMN ')[1]}")
        print(f"Applied {len(migrations)} v12 migration(s) to users table")
    else:
        print("v12 schema is up to date")


def _run_v14_migrations(engine):
    """Add v14 columns to existing tables.

    v14 adds a 'tier' column to the users table for tiered rate limiting.
    New tables (semantic_cache, embedding_cache, budget_alerts, audit_logs,
    api_keys) are created by Base.metadata.create_all().
    """
    inspector = inspect(engine)
    ...
    if migrations:
        with engine.begin() as conn:
            for sql in migrations:
                conn.execute(text(sql))
                print(f"  Migration: {sql.split('ADD COLUMN ')[1]}")
        print(f"Applied {len(migrations)} v14 migration(s) to users table")
    else:
        print("v14 schema is up to date")
...
def init_database():
    ...
    _run_v12_migrations(engine)
    _run_v14_migrations(engine)
    ...
    try:
        ...
        if not default_user:
            ...
            db.commit()
            print("Created default user (email: default@studybuddy.local, password: studybuddy)")
        else:
            # Backfill email/password for existing default user if missing
            if not default_user.email:
                from ..auth import hash_password
                default_user.email = "default@studybuddy.local"
                default_user.password_hash = hash_password("studybuddy")
                default_user.is_verified = True
                db.commit()
                print("Updated default user with auth credentials")
    finally:
        db.close()
