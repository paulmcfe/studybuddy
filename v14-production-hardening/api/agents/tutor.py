"""LangGraph-based tutor agent for StudyBuddy v14.

v14 adds content guardrail nodes to the agent graph:
- input_guard: Checks user messages for prompt injection and inappropriate content
- output_guard: Filters agent responses for educational safety

Graph structure:
  START → input_guard → [blocked?] → agent → [tools_condition] → tools → agent
                                                                → output_guard → END
                       → END (if blocked, with refusal message)
...
from ..services.guardrails import input_guardrail, output_guardrail, GuardrailResult
...
def _build_langgraph_agent(system_prompt: str, tools: list):
    """Build a LangGraph agent with guardrail nodes.

    Graph structure:
      START → input_guard → [pass/blocked] → agent → [tools_condition] → tools → agent
                                                                       → output_guard → END
                          → END (blocked, with refusal message)
    """
    ...
    async def input_guard(state: MessagesState):
        ...
        if last_human:
            check = input_guardrail.check(last_human.content)
            if check.result == GuardrailResult.BLOCKED:
                return {"messages": [AIMessage(content=check.reason)]}

        return {"messages": []}

    def route_after_guard(state: MessagesState):
        """Route based on whether the guardrail blocked the input."""
        messages = state["messages"]
    ...
    async def output_guard(state: MessagesState):
        """Filter the agent's response for safety."""
        messages = state["messages"]
        if messages and isinstance(messages[-1], AIMessage):
            last_ai = messages[-1]
            if last_ai.content:
                check = output_guardrail.filter(last_ai.content)
                if check.result == GuardrailResult.MODIFIED:
                    return {"messages": [AIMessage(content=check.filtered_text)]}
        return {"messages": []}
    ...
    builder.add_conditional_edges("input_guard", route_after_guard, ["agent", "output_guard"])
...
def create_tutor_graph():
    ...
    Includes guardrail nodes for production safety.
    """
    from langchain_openai import ChatOpenAI
    from langchain_core.messages import SystemMessage, HumanMessage, AIMessage
    from langgraph.graph import StateGraph, MessagesState, START, END
