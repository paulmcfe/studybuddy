"""Document indexing service for StudyBuddy v11.

Handles document parsing, chunking, and indexing into program-specific
Qdrant collections.

v11 adds index_text_content() for indexing content from MCP connectors
(URLs and GitHub files) without requiring a file on disk.
"""
...
async def index_document_to_program(
    ...
    # Get vector store (v14: uses cached embeddings via retrieval.py)
    vector_store = get_program_vector_store(program)

    # v14: Batch processing for large documents (100 chunks per batch)
    BATCH_SIZE = 100
