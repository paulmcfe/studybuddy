...
def get_program_vector_store(program: LearningProgram) -> QdrantVectorStore:
    ...
    v14: Uses DatabaseBackedEmbeddings to cache embedding vectors and
    avoid redundant OpenAI API calls for previously seen content.
    """
    ensure_collection_exists(program.qdrant_collection)

    # v14: Use cached embeddings wrapper
    try:
        from .cached_embeddings import get_cached_embeddings
        from ..database.connection import SessionLocal
        embeddings = get_cached_embeddings(SessionLocal)
    except Exception:
        # Fallback to direct embeddings if cache not available
