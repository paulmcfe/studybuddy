"""StudyBuddy v14 - Production Hardening API

FastAPI application with:
- Content guardrails: prompt injection detection, output filtering (v14)
- Semantic response caching and embedding caching (v14)
- Cost analytics with budget alerts (v14)
- Distributed security: API keys, audit logging, TLS (v14)
- Tiered rate limiting with graceful degradation (v14)
- Production checklist and alerting (v14)
- Multi-model support: OpenAI, Together AI, Ollama (v13)
- Per-task model configuration with fallback chains (v13)
- Cost tracking and comparison dashboard (v13)
- Performance benchmarking across providers (v13)
- MCP Server exposing StudyBuddy capabilities (v13)
- Agent-to-Agent discovery via AgentCard (v13)
...
from .database.models import (
    ModelConfig, TokenUsage, BenchmarkResult,
    SemanticCache, EmbeddingCache, BudgetAlert, AuditLog, ApiKey,
)
from .auth import (
    get_current_user,
    hash_password,
...
from .rate_limit import limiter, dynamic_limit
from .services.monitoring import metrics, normalize_path
from .services.guardrails import input_guardrail, GuardrailResult
...
# Create FastAPI app
app = FastAPI(
    title="StudyBuddy v14",
    description="Production-hardened learning application with guardrails, caching, distributed security, tiered rate limiting, and cost analytics",
    version="14.0.0",
)
...
# v14: Tier middleware — sets request.state.user_tier for tiered rate limiting
@app.middleware("http")
async def tier_middleware(request: Request, call_next):
    """Extract user tier from JWT and attach to request state."""
    request.state.user_tier = "free"  # default
        ...
        try:
            ...
            if user_id:
                ...
                try:
                    ...
                    if user:
                        request.state.user_tier = getattr(user, "tier", "free") or "free"
                finally:
                    db.close()
        except Exception:
            pass
...
@limiter.limit(dynamic_limit("auth"))
def register(
...
@limiter.limit(dynamic_limit("document_upload"))
...
@limiter.limit(dynamic_limit("flashcard_generate"))
async def generate_flashcard_endpoint(
    ...
    # v14: Input guardrail check on topic
    if request_obj and request_obj.topic:
        guardrail_check = input_guardrail.check(request_obj.topic)
        if guardrail_check.result == GuardrailResult.BLOCKED:
...
async def chat_with_tutor(
    ...
    # v14: Input guardrail check
    guardrail_check = input_guardrail.check(request_obj.message)
    if guardrail_check.result == GuardrailResult.BLOCKED:
        raise HTTPException(status_code=400, detail=guardrail_check.reason)

    # v14: Check semantic cache (only for non-streaming-required cases)
    cached_response = await semantic_cache.get(
        query=request_obj.message,
        program_id=program_id,
        task_type="tutoring",
        db=db,
    )
    if cached_response:
        ...
        async def stream_cached():
            yield f"data: {_json.dumps(cached_response)}\n\n"
            yield "data: [DONE]\n\n"
...
If the context doesn't contain relevant information, say so honestly."""
    ...
    async def generate():
        """Stream the response chunks and cache the full response."""
        import json
        full_response_parts = []
        ...
        # v14: Cache the accumulated response
        ...
        if full_response:
            try:
                ...
                logger.warning("Failed to cache chat response", exc_info=True)
...
class ModelConfigUpdate(BaseModel):
    ...
    task_type: str  # flashcard_generation, tutoring, curriculum, embedding
    primary_provider: str  # openai, together, ollama
    primary_model: str
...
def list_available_models():
    ...
    Returns models from OpenAI, Together AI, and Ollama with
    availability status based on configuration.
    """
...
def get_model_config(
    ...
    """Get model configuration for the current user."""
    ...
    from .services.models.config import DEFAULT_TASK_MODELS, TaskType, AVAILABLE_MODELS

    for task_type in TaskType:
        if task_type.value not in result:
            defaults = DEFAULT_TASK_MODELS.get(task_type, {})
            primary_key = defaults.get("primary", "gpt-4o-mini")
            fallback_key = defaults.get("fallback")

            primary_info = AVAILABLE_MODELS.get(primary_key)
            fallback_info = AVAILABLE_MODELS.get(fallback_key) if fallback_key else None

            result[task_type.value] = {
                "primary_provider": primary_info.provider.value if primary_info else "openai",
                "primary_model": primary_info.model_id if primary_info else "gpt-4o-mini",
                "fallback_provider": fallback_info.provider.value if fallback_info else None,
                "fallback_model": fallback_info.model_id if fallback_info else None,
                "temperature": 0.7,
                "max_tokens": None,
                "is_default": True,
            }
...
def update_model_config(
    ...
    existing = db.query(ModelConfig).filter(
        ModelConfig.user_id == current_user.id,
        ModelConfig.task_type == config.task_type,
    ).first()

    if existing:
        existing.primary_provider = config.primary_provider
        existing.primary_model = config.primary_model
        existing.fallback_provider = config.fallback_provider
        existing.fallback_model = config.fallback_model
    ...
    return {"message": "Configuration updated", "task_type": config.task_type}
...
def get_cost_summary(
    ...
    """Get cost summary for the current user over a time period."""
    ...
    usage = db.query(
        ...
        func.sum(TokenUsage.cost_cents).label("total_cost_cents"),
        func.count(TokenUsage.id).label("call_count"),
        func.count(func.nullif(TokenUsage.was_fallback, False)).label("fallback_count"),
    ).filter(
    ...
    for row in usage:
        cost = {
            ...
            "cost_usd": (row.total_cost_cents or 0) / 100,
...
@app.get("/api/costs/comparison")
def get_cost_comparison(
    input_tokens: int = Query(default=10_000_000),
    output_tokens: int = Query(default=5_000_000),
):
    """Compare monthly costs across providers for hypothetical usage."""
    from .services.models.benchmark import compare_costs

    comparison = compare_costs(input_tokens, output_tokens)
...
@limiter.limit(dynamic_limit("benchmark"))
async def run_benchmark(
    request: Request,
    prompt: str = Query(default="Explain spaced repetition in 3 sentences."),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db_dependency),
):
    """Run performance benchmark across available models.

    Rate limited to 5 runs per hour to prevent abuse.
    """
    from .services.models.benchmark import run_benchmark_suite

    results = await run_benchmark_suite(prompt=prompt)
...
def get_agent_card():
    ...
    Other agents can use this endpoint to discover what
    StudyBuddy can do and how to communicate with it.
    """
    from .mcp_server.agent_card import get_agent_card_dict

    return get_agent_card_dict()


# ============== v14: Cost Analytics Endpoints ==============


@app.get("/api/costs/by-feature")
def get_costs_by_feature(
    days: int = Query(default=30, ge=1, le=365),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db_dependency),
):
    """Get per-feature cost breakdown aggregated by task_type."""
    from sqlalchemy import func
    ...
    return [
        {
            ...
            "total_cost_cents": float(r.total_cost_cents or 0),
...
def get_costs_daily(
    ...
    results = (
        db.query(
            ...
            func.sum(TokenUsage.cost_cents).label("cost_cents"),
        )
        .filter(TokenUsage.user_id == current_user.id)
        .filter(TokenUsage.created_at >= cutoff)
    ...
    return [
        {
            "date": str(r.date),
            "cost_cents": float(r.cost_cents or 0),
        }
        for r in results
    ]


# ============== v14: Budget Endpoints ==============
...
def get_budget(
    ...
    """Get current user's budget configuration."""
    ...
    if not alert:
        return {"monthly_budget_cents": 0, "warning_threshold_pct": 80, "is_active": False}

    return {
        "monthly_budget_cents": alert.monthly_budget_cents,
        "warning_threshold_pct": alert.warning_threshold_pct,
...
def set_budget(
    ...
    if alert:
        alert.monthly_budget_cents = request_obj.monthly_budget_cents
        alert.warning_threshold_pct = request_obj.warning_threshold_pct
        alert.is_active = True
    else:
        alert = BudgetAlert(
            id=str(uuid.uuid4()),
            user_id=current_user.id,
            monthly_budget_cents=request_obj.monthly_budget_cents,
            warning_threshold_pct=request_obj.warning_threshold_pct,
            is_active=True,
        )
        db.add(alert)

    db.commit()
    return {"status": "ok", "monthly_budget_cents": alert.monthly_budget_cents}
...
def get_budget_status(
    ...
    spend = db.query(func.sum(TokenUsage.cost_cents)).filter(
        TokenUsage.user_id == current_user.id,
        TokenUsage.created_at >= month_start,
    ).scalar() or 0

    budget_cents = alert.monthly_budget_cents if alert else 0
    pct_used = (float(spend) / budget_cents * 100) if budget_cents > 0 else 0
    alert_triggered = alert and pct_used >= alert.warning_threshold_pct if alert else False

    return {
        "spend_cents": float(spend),
...
# ============== v14: API Key Management Endpoints ==============
...
def create_api_key_endpoint(
    request_obj: ApiKeyCreateRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db_dependency),
):
    ...
    expires_at = None
    if request_obj.expires_in_days:
        expires_at = datetime.utcnow() + timedelta(days=request_obj.expires_in_days)
...
def list_api_keys(
    ...
    """List API keys (prefix and name only, never full key)."""
...
# ============== v14: Production Checklist Endpoint ==============


@app.get("/api/production-checklist")
def get_production_checklist(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db_dependency),
):
    """Run production readiness checks and return results."""
    from .services.production_checklist import ProductionChecklist
...
def get_alerts(
    ...
    from .services.alerting import alert_evaluator

    triggered = alert_evaluator.evaluate(db, current_user.id)

    # Update metrics
    metrics.set_active_alerts(len(triggered))
...
def get_alert_rules(
    ...
    from .services.alerting import DEFAULT_ALERT_RULES
