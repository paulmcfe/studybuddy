...
type View = 'overview' | 'study' | 'flashcards' | 'documents' | 'connectors' | 'new-program' | 'models' | 'costs' | 'benchmarks' | 'cost-analytics' | 'checklist'
    ...
    return (
        <aside className="sidebar">
            <div className="sidebar-header">
                ...
                <span className="sidebar-version">v14 - Production Hardening</span>
            </div>
            ...
            <nav className="sidebar-nav">
                <h3 className="sidebar-nav-title">Infrastructure</h3>
                <button
                    onClick={() => onNavigate('models')}
                    className={`sidebar-nav-btn tab ${currentView === 'models' ? 'active' : ''}`}
                >
                    Models
                </button>
                <button
                    onClick={() => onNavigate('costs')}
                    className={`sidebar-nav-btn tab ${currentView === 'costs' ? 'active' : ''}`}
                >
                    Costs
                </button>
                <button
                    onClick={() => onNavigate('benchmarks')}
                    className={`sidebar-nav-btn tab ${currentView === 'benchmarks' ? 'active' : ''}`}
                >
                    Benchmarks
                </button>
                <button
                    onClick={() => onNavigate('cost-analytics')}
                    className={`sidebar-nav-btn tab ${currentView === 'cost-analytics' ? 'active' : ''}`}
                >
                    Cost Analytics
                </button>
                <button
                    onClick={() => onNavigate('checklist')}
                    className={`sidebar-nav-btn tab ${currentView === 'checklist' ? 'active' : ''}`}
                >
