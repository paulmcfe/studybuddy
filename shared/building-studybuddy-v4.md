...
StudyBuddy v4's knowledge base consists of nearly three dozen reference guides, all markdown files covering RAG, agents, LangGraph, embeddings, tool use, memory systems, and more. These files should be placed in a documents folder:
...
```json
{
    ...
    "builds": [
        {
            "src": "v4-agentic-rag/api/index.py",
            "use": "@vercel/python"
        },
        {
            "src": "v4-agentic-rag/frontend/package.json",
            "use": "@vercel/next"
        }
    ],
    "routes": [
        { "src": "/api/(.*)", "dest": "/v4-agentic-rag/api/index.py" },
        { "src": "/(.*)", "dest": "/v4-agentic-rag/frontend/$1" }
    ],
    "git": {
        "deploymentEnabled": {
            "main": true
...
Second, update scripts/should-deploy.sh to trigger on v4 changes:

```bash
#!/bin/bash
# Deploy if changes are in v4-agentic-rag or vercel config
# Exit 1 (true) = proceed with build, Exit 0 (false) = skip build
git diff HEAD^ HEAD --name-only | grep -qE "^v4-agentic-rag/|^vercel\.json" && exit 1 || exit 0
```
...
Or simply push to GitHub. Vercel will automatically deploy when changes are detected in the v4-agentic-rag directory.
