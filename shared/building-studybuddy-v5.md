...
Notice we're using gpt-4o-mini for the Card Generator. Card generation benefits from structured output capabilities, and gpt-4o-mini handles JSON generation well. The Tutor uses gpt-4o for conversational quality. Different agents can use different models based on their needs.
...
StudyBuddy v5 runs with two terminals—one for the backend, one for the frontend.
...
To deploy v5 to Vercel, you need to update two files in the repo root.

First, update vercel.json to point to the v5-multi-agent directory:
...
```json
{
    ...
    "builds": [
        {
            "src": "v5-multi-agent/api/index.py",
            "use": "@vercel/python"
        },
        {
            "src": "v5-multi-agent/frontend/package.json",
            "use": "@vercel/next"
        }
    ],
    "routes": [
        { "src": "/api/(.*)", "dest": "/v5-multi-agent/api/index.py" },
        { "src": "/(.*)", "dest": "/v5-multi-agent/frontend/$1" }
    ],
    "git": {
        "deploymentEnabled": {
            "main": true
...
Second, update scripts/should-deploy.sh to trigger on v5 changes:
...
Or simply push to GitHub. Vercel will automatically deploy when changes are detected in the v5-multi-agent directory.
